...
이 프로젝트의 목표는 기능 수를 늘리는 것이 아니라 Agent 런타임의 설계 결정을 직접 구현하고 비교하며 설명할 수 있게 되는 것이다. 계층을 모두 만든 뒤 연결하지 않고, 매 단계마다 사용자 요청에서 응답까지
동작하는 **수직 슬라이스**를 완성한다.
...
현재 상태: **완료 — 단일 인스턴스·메모리 저장소 범위**. 동기 LLM transport를 물리적으로 중단하는 기능은 Phase 6에서 다루며, 현재는 취소 후 늦게 도착한 결과를 폐기하는 협력적 취소를
사용한다.
