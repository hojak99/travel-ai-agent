...
| 05 | QueryEngine Facade와 query Kernel | 01, 02 | 세션 단위 Facade와 한 요청의 실행 Loop |
| 06 | System Prompt의 섹션 조립 | 03, 11 | 여행 규칙·State·Tool 설명의 안정적 조립 |
| 07 | 다단계 Compaction과 복구 | 04 | Microcompact, Full Compact, State 재구성 |
| 08 | Prompt Cache의 정적·동적 경계 | 03, 11 | 고정 Prompt와 세션별 Context 분리 |
| 10 | Tool Protocol, Registry, ToolSearch | 05 | 등록·검색·권한 확인·실행 분리 |
...
| 16 | Task 모델과 TaskType 계보 | 07 | 작업 상태와 실행 타입을 구조화 |
| 17 | Coordinator와 예약 실행 | 07 | Coordinator 중심 실행과 향후 일정 알림 |
| 18 | MCP Protocol 구현 | 08 | 날씨·지도·캘린더 외부 Tool |
| 19 | Permission과 승인 결과 전달 | 08, 10 | 예약·결제·캘린더 등록 승인 |
| 20 | Hooks | 10 | 실행 전·후·중단·오류 이벤트 확장점 |
| 21 | Skill·Plugin·Output Style | 12 | 여행 스타일과 도메인 확장점 |
| 23 | Transport와 API Retry | 02, 10 | SSE, timeout, retry, fallback |
| 31 | 다층 Memory | 09 | 세션·여행 계획·사용자 선호·장기 기억 |
| 33 | Store와 UI/비즈니스 상태 연결 | 09, 10 | React 상태와 Agent Runtime 상태 연결 |
| 34 | 이식 가능한 11가지 패턴 | 12 | 프로젝트의 설계 선택과 트레이드오프 설명 |
