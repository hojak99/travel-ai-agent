...
| 발견한 공백 | 반영 문서 |
|---|---|
| Prompt 섹션·정적/동적 경계·구조화 Decision | 03, 11 |
| 등급별 Compaction과 재구성·실패 차단 | 04 |
| Tool 활성화·위험 등급·병렬성·멱등성 | 05 |
| Sub-agent 생성 기준·격리·취소 전파 | 06 |
| Workflow/Task/Decision 구분과 Checkpoint | 07 |
| MCP 연결 생명주기와 Permission 결합 | 08 |
| Memory 수명·회상·개인정보 정책 | 09 |
| Event replay·승인 payload·Hook 실패 정책 | 10 |
| 수직 슬라이스 구현과 학습 실험 | 16 |
| Prompt Injection, SSRF, 평가 피라미드, 출시 관문 | 17 |

이 문서는 차이 분석의 근거로 유지한다. 실제 구현 순서는 [16 구현·학습 로드맵](./16-implementation-learning-roadmap.md), 안전성과 품질 기준은 [17 평가와 보안](./17-evaluation-and-security.md)을 따른다.
